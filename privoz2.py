...
def get_exchange_rates():
    # Список источников
    urls = [
        "https://www.cbr-xml-daily.ru/daily_utf8.xml", # Основное зеркало
        "https://www.floatrates.com/daily/rub.xml"      # Запасной международный источник
    ]
    ...
    for url in urls:
        try:
            ...
            # Логика для разных источников
            if "cbr-xml-daily" in url:
                eur = float(df.loc[df['CharCode'] == 'EUR', 'Value'].values[0].replace(',', '.'))
                usd = float(df.loc[df['CharCode'] == 'USD', 'Value'].values[0].replace(',', '.'))
                cny = float(df.loc[df['CharCode'] == 'CNY', 'Value'].values[0].replace(',', '.'))
            else:
                # Логика для floatrates (там курсы к рублю идут иначе)
                eur = 1 / float(df.loc[df['targetCurrency'] == 'EUR', 'exchangeRate'].values[0])
                usd = 1 / float(df.loc[df['targetCurrency'] == 'USD', 'exchangeRate'].values[0])
                cny = 1 / float(df.loc[df['targetCurrency'] == 'CNY', 'exchangeRate'].values[0])
            
            return eur, usd, cny
        except:
            continue # Если этот сайт не ответил, пробуем следующий
